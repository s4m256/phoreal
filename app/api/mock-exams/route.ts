...
export async function POST(request:Request) {
  ...
  const statements:D1PreparedStatement[]=[db.prepare("INSERT INTO user_mock_exams_v2 (id,owner_id,exam_name,date,type,total_score,drive_url) VALUES (?,?,?,?,?,?,?)").bind(id,user.userId,examName,payload.date,payload.type,payload.totalScore,payload.driveUrl?.trim()||null)];
