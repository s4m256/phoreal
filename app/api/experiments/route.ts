...
export async function POST(request:Request) {
  ...
  const id=crypto.randomUUID(); await getD1().prepare("INSERT INTO user_experiments (id,title,date,image_url,notes) VALUES (?,?,?,?,?)").bind(id,title,payload.date||null,payload.imageUrl?.trim()||null,payload.notes?.trim()||null).run();
