...
export async function POST(request: Request) {
  ...
  const existing = await db.prepare("SELECT * FROM user_attempts WHERE owner_id=? AND problem_id=? AND status='in_progress' LIMIT 1").bind(user.userId,problemId).first();
  if (existing) return Response.json({ attempt: existing, resumed: true });
  const id = crypto.randomUUID(); const now = new Date().toISOString();
  await db.prepare("INSERT INTO user_attempts (id,owner_id,problem_id,status,current_state,started_at,updated_at) VALUES (?,?,?,'in_progress','paused',?,?)").bind(id,user.userId,problemId,now,now).run();
