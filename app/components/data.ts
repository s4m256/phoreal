...
export function secondsFor(segment:Pick<Segment,"duration_seconds"|"started_at">, now=Date.now()){if(segment.duration_seconds!=null)return Number(segment.duration_seconds);return Math.max(0,Math.floor((now-Date.parse(segment.started_at))/1000));}
