...
for (const row of problems) {
  sql.push(statement(`INSERT INTO phors_problems (exam_id,source_id,source_url,code,title,title_pt,kind,statement_url,solution_url,marking_scheme_url,statement_pdf_url,solution_pdf_url,attachments_json,statement_html_source,statement_html_original,statement_text_original,statement_content_hash,statement_language,statement_status,statement_html_pt,translation_status,translation_source_hash,parts_status,source_hash,imported_at,created_at,updated_at) VALUES ((SELECT id FROM phors_exams WHERE source_url=${quote(examSourceById.get(row.exam_id))}),${quote(row.source_id)},${quote(row.source_url)},${quote(row.code)},${quote(row.title)},${quote(row.title_pt)},${quote(row.kind)},${quote(row.statement_url)},${quote(row.solution_url)},${quote(row.marking_scheme_url)},${quote(row.statement_pdf_url)},${quote(row.solution_pdf_url)},${quote(row.attachments_json)},${quote(row.statement_html_source)},${quote(row.statement_html_original)},${quote(row.statement_text_original)},${quote(row.statement_content_hash)},${quote(row.statement_language)},${quote(row.statement_status)},${quote(row.statement_html_pt)},${quote(row.translation_status)},${quote(row.translation_source_hash)},${quote(row.parts_status)},${quote(row.source_hash)},${quote(row.imported_at)},${quote(row.created_at)},${quote(row.updated_at)}) ON CONFLICT(source_id) DO UPDATE SET exam_id=excluded.exam_id,source_url=excluded.source_url,code=excluded.code,title=excluded.title,title_pt=COALESCE(excluded.title_pt,phors_problems.title_pt),kind=excluded.kind,statement_url=excluded.statement_url,solution_url=excluded.solution_url,marking_scheme_url=excluded.marking_scheme_url,statement_pdf_url=excluded.statement_pdf_url,solution_pdf_url=excluded.solution_pdf_url,attachments_json=excluded.attachments_json,statement_html_source=excluded.statement_html_source,statement_html_original=excluded.statement_html_original,statement_text_original=excluded.statement_text_original,statement_content_hash=excluded.statement_content_hash,statement_language=excluded.statement_language,statement_status=excluded.statement_status,statement_html_pt=CASE WHEN phors_problems.translation_status='verified' AND phors_problems.translation_source_hash=excluded.statement_content_hash THEN phors_problems.statement_html_pt ELSE excluded.statement_html_pt END,translation_status=CASE WHEN phors_problems.translation_status='verified' AND phors_problems.translation_source_hash=excluded.statement_content_hash THEN phors_problems.translation_status ELSE excluded.translation_status END,translation_source_hash=CASE WHEN phors_problems.translation_status='verified' AND phors_problems.translation_source_hash=excluded.statement_content_hash THEN phors_problems.translation_source_hash ELSE excluded.translation_source_hash END,parts_status=excluded.parts_status,source_hash=excluded.source_hash,imported_at=excluded.imported_at,updated_at=excluded.updated_at`));
}
