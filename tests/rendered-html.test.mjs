...
test("uses the personal training shell",async()=>{
  const [layout,page,dashboard,pkg]=await Promise.all([read("app/layout.tsx"),read("app/page.tsx"),read("app/components/Dashboard.tsx"),read("package.json")]);assert.match(layout,/title: "PhoReal"/);assert.match(layout,/>PhoReal<\/Link>/);assert.match(layout,/href="\/problemas">Problemas/);assert.doesNotMatch(layout,/Entrar com ChatGPT/);assert.match(dashboard,/Explorar problemas/);assert.match(page,/Dashboard/);assert.doesNotMatch(`${layout}${page}${pkg}`,/codex-preview|SkeletonPreview/);
});

test("loads all Taiwan volumes with figures, parts and full training UI",async()=>{
  const [page,workspace,raw,catalogRaw,manifestRaw]=await Promise.all([read("app/problemas/taiwan/[volume]/[number]/page.tsx"),read("app/components/TaiwanProblemWorkspace.tsx"),read("data/taiwan/volume-10.json"),read("data/taiwan/catalog.json"),read("data/taiwan/import-manifest.json")]);const data=JSON.parse(raw);const catalog=JSON.parse(catalogRaw);const manifest=JSON.parse(manifestRaw);
  const totals=manifest.volumes.reduce((sum,volume)=>({problems:sum.problems+volume.problems,parts:sum.parts+volume.parts,figures:sum.figures+volume.images,solutions:sum.solutions+volume.solutions}),{problems:0,parts:0,figures:0,solutions:0});
  assert.equal(catalog.volumes.length,10);assert.deepEqual(totals,{problems:296,parts:1747,figures:349,solutions:291});
  ...
  assert.ok(data.problems.every((problem)=>problem.title_pt&&problem.statement_html));assert.doesNotMatch(raw,/<br><br>tial/);assert.match(page,/TaiwanProblemWorkspace/);assert.match(page,/volume=\{volumeNumber\}/);assert.match(workspace,/onPartClick=\{selectPart\}/);assert.match(workspace,/Mostrar resposta/);assert.match(workspace,/api\/taiwan\/attempts/);
...
test("uses a readable print serif only for statement and solution bodies",async()=>{
  const css=await read("app/globals.css");
  assert.match(css,/\.problem-page \.statement-panel \.statement-content,\.problem-page \.solution-panel \.statement-content\{font-family:Georgia,"Times New Roman",serif/);
  assert.doesNotMatch(css,/\.problem-header h1[^}]*font-family:(?:Georgia|"Times New Roman")/);
});
